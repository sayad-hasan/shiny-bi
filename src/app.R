...
ui <- secure_app(
    tags_top =
        tags$div(
            tags$img(
                src = "logo/shinyLogo.png", width = 150
            ),
    ...
    tags_bottom = tags$div(
        tags$p(
            ...
            tags$a(
                ...
                target="_top", "Shiny Insurance Company"
    ...
    dashboardPage(
        skin = "green",
        dashboardHeader(title = "Shiny Insurance BI"
                        # tags$li(
                        #     h4(textOutput("time_updated_header"), align = "right",
                        #        style={'color: #ffffff; text-align: center;margin-right: 70px!important;'}),
                        #     class = "dropdown",
                        #     # style = ""
                        # )
                        ),
...
server <- function(input, output, session) {
    ...
    output$time_updated_header <- renderText({
        paste0("Updated on: ",weekdays(time_updated), ", ", time_updated)
    })
