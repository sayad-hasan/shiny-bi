...
output$companyname_all_tab <- renderUI({
  if(input$selectinput == "All"){
    name <- "All Branches"
  }
  else {
    name <-  paste(input$selectinput, "Life Insurance")
  }
  
  h3(
    paste0("Showing result for ", name),
...
output$age_dist <- renderHighchart({
    ...
    hchart(age_group_hc,
           'column', hcaes(x = Age_Group, y = Frequecy, color = Age_Group), name = "total"
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
...
output$linechart <- renderHighchart({
    ...
    hchart(data,
           'line', hcaes(x = policystartdate, y = total, color = total), name = "total"
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
      ...
      # hc_xAxis(title = list(text = "date")) %>%
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = "Total Policies Issued Per Day"
        )
...
output$monthlychart <- renderHighchart({
    ...
    hchart(total_by_month,
           'column', hcaes(x = month, y = total, color = month), name = "total"
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste("Total Policies Issued Per Month", input$selectyear)
        )
...
output$yearlychart <- renderHighchart({
    ...
    hchart(total_by_year,
           'column', hcaes(x = year, y = total, color = year), name = "total"
    ) %>%
        # hc_plotOptions(
        #     column = list(
        #         dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = "Total Policies Issued Per Year"
        )
...
output$totalpaidup <- renderUI({
  ...
  sub <- ( paste(compress(totalpaidup), "Total Paidup Lapsed Policy"))
...
output$policysummary_download <- downloadHandler(
  filename = function() {
    
    if(input$selectinput == "All"){
      paste0("All_Company_Policy_Status", ".csv")
    }
    else {
      paste0(input$selectinput,"_Policy_Status" , ".csv")
    }
  ...
  content = function(file) {
    ...
    if(input$selectinput == "All"){
      ...
      write.csv(policy_status, file, row.names = F)
    }
    else {
      write.csv(policy_status %>% filter(company_name == input$selectinput), file, row.names = F)
    }
...
output$linechart_r <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$linechart_r_ortype, 
             "Total Premium" = {
             ...
             "First Premium" = {
               ...
               data <- premium_by_ortype_count %>% 
                 filter(ortype == "F",
                        ordate %in% seq(input$daterange_r[1],
                                        input$daterange_r[2], by = 'day')) %>%
                 group_by(ordate) %>% 
                 summarise(total = sum(total, na.rm = T))
               
               # premium_by_ortype_count
             }, 
             "Deferred Premium" = {
               data <- premium_by_ortype_count %>% 
                 filter(ortype == "D",
    ...
    hchart(data,
           'line', hcaes(x = ordate, y = total, color = total), name = input$linechart_r_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$linechart_r_ortype, "E-Receipt Issued Per Day")
        )
...
output$monthlychart_r <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$monthlychart_r_ortype, 
             "Total Premium" = {
             ...
             "First Premium" = {
               total_by_month <- premium_by_ortype_count %>%
                 filter(ortype == "F",
                        year(ordate) == input$selectyear_r) %>%
                 group_by(month_r = month(ordate, label =T)) %>% summarise(total = sum(total, na.rm = T))
               # premium_by_ortype_count
             }, 
             "Deferred Premium" = {
               total_by_month <- premium_by_ortype_count %>%
    ...
    hchart(total_by_month,
        'column', hcaes(x = month_r, y = total, color = month_r), name = input$monthlychart_r_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
      ...
      hc_xAxis(title = list(text = "month")) %>%
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$monthlychart_r_ortype, "E-Receipt Issued Per Month", input$selectyear_r)
        )
...
output$yearlychart_r <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$yearlychart_r_ortype, 
             "Total Premium" = {
               total_by_month <- monthlychart_r %>%
             ...
             "First Premium" = {
               total_by_month <- premium_by_ortype_count %>%
                 filter(ortype == "F") %>%
                 group_by(year_r = year(ordate)) %>% summarise(total = sum(total, na.rm = T))
             }, 
             "Deferred Premium" = {
               total_by_month <- premium_by_ortype_count %>%
                 filter(ortype == "D") %>%
    ...
    hchart(total_by_month,
           'column', hcaes(x = year_r, y = total, color = year_r), name = input$yearlychart_r_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$yearlychart_r_ortype, "E-Receipt Issued Per Year")
        )
...
#### DOWNLOAD E-reciept Summary

output$e_receipt_download <- downloadHandler(
  filename = function() {
    
    if(input$selectinput == "All"){
      paste0("All_Company_E_Receipt_Summary_", input$daterange_r[1],"_to_", input$daterange_p[2],".csv")
    }
    else {
      paste0(input$selectinput,"_E_Receipt_Summary_",input$daterange_r[1],"_to_", input$daterange_p[2], ".csv")
    }
  ...
  content = function(file) {
    ...
    if(input$selectinput == "All"){
      data <- premium_by_ortype_count %>%
        filter(ordate %in% seq(input$daterange_r[1],
                               input$daterange_r[2], by = 'day')) %>%
        group_by(company_name, ortype) %>% summarise(E_Receipt = sum(total))
...
#### DOWNLOAD Premium Summary

output$premiumsummary_download <- downloadHandler(
  filename = function() {
    
    if(input$selectinput == "All"){
      paste0("All_Company_Premium_Summary_", input$daterange_p[1],"_to_", input$daterange_p[2],".csv")
    }
    else {
      paste0(input$selectinput,"_Premium_Summary_",input$daterange_p[1],"_to_", input$daterange_p[2], ".csv")
    }
  ...
  content = function(file) {
    ...
    if(input$selectinput == "All"){
      data <- premium_by_ortype_sum %>%
        filter(ordate %in% seq(input$daterange_p[1],
                               input$daterange_p[2], by = 'day')) %>%
        group_by(company_name, ortype) %>% summarise(premium_collection = sum(total))
...
output$linechart_p <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$linechart_p_ortype, 
             "Total Premium" = {
               data <- linechart_p_or %>%
                 filter(ordate %in% seq(input$daterange_p[1],
                                        input$daterange_p[2], by = 'day')) %>%
      ...
      switch(input$linechart_p_ortype, 
             "Total Premium" = {
               data <- linechart_p_or %>%
                 filter(company_name == input$selectinput,
                        ordate %in% seq(input$daterange_p[1],
                                        input$daterange_p[2], by = 'day'))
             } , 
             "First Premium" = {
               data <- premium_by_ortype_sum %>%
    ...
    hchart(data,
           'line', hcaes(x = ordate, y = total, color = total) , name = input$linechart_p_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$linechart_p_ortype, "Collection Per Day")
        )
...
# %>% bindCache(input$selectinput, input$daterange_p, input$linechart_p_ortype)
...
output$monthlychart_p <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$monthlychart_p_ortype, 
             "Total Premium" = {
             ...
             "First Premium" = {
               total_by_month_p <- premium_by_ortype_sum %>%
                 filter(year(ordate) == input$selectyear_p, 
                        ortype == "F") %>%
                 group_by(month_r = month(ordate, label =T)) %>% summarise(total = sum(total, na.rm = T))
             }, 
             "Deferred Premium" = {
      ...
      switch(input$monthlychart_p_ortype, 
             ...
             "First Premium" = {
               total_by_month_p <- premium_by_ortype_sum %>%
                 filter(company_name == input$selectinput, 
                        year(ordate) == input$selectyear_p, 
                        ortype == "F") %>%
                 group_by(month_r = month(ordate, label =T)) %>% summarise(total = sum(total, na.rm = T))
             }, 
             "Deferred Premium" = {
    ...
    hchart(total_by_month_p,
           'column', hcaes(x = month_r, y = total, color = month_r), name = input$monthlychart_p_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$monthlychart_p_ortype, "Collection Per Month", input$selectyear_p)
        )
...
output$yearlychart_p <- renderHighchart({
    ...
    if(input$selectinput == "All"){
      ...
      switch(input$yearlychart_p_ortype, 
             "Total Premium" = {
             ...
             "First Premium" = {
               ...
               total_by_year_p <- premium_by_ortype_sum %>%
                 filter(ortype == "F") %>%
                 group_by(year_r = year(ordate)) %>% summarise(total = sum(total, na.rm = T)) 
    ...
    hchart(total_by_year_p,
           'column', hcaes(x = year_r, y = total, color = year_r), name = input$yearlychart_p_ortype
    ) %>%
        hc_plotOptions(
            column = list(
                dataLabels = list(
        ...
        hc_exporting(
            enabled = TRUE, # always enabled
            filename = paste(input$yearlychart_p_ortype, "Collection Per Year")
        )
